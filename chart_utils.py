...
def moving_average(x, w):
    ...
    return [np.convolve(x[0], np.ones(w), 'same') / w]
...
def compute_conf_intervals(data, cumulative=False):
    ...
    for i, all_instances in enumerate(data):
        ...
        alg_i_ci = []
        total_so_far = 1#np.zeros(num_instances)
...
def plot(results, experiment_dir, agents, plot_file_name="", conf_intervals=[], use_cost=False, cumulative=False, episodic=True, open_plot=True, track_disc_reward=False, plot_title = '', plot_y = 'Reward'):
    '''
    Args:
    ...
    x_axis_unit = "episode" if episodic else "Epoch"
    ...
    agent_colors = 0 #_get_agent_colors(experiment_dir, agents)

    # Make the plot.
    print_prefix = "\nAvg. cumulative reward" if cumulative else plot_y
    # For each agent.
    for i, agent_name in enumerate(agents):
        print('i:', i , agent_name)
        # Add figure for this algorithm.
        agent_color_index = 0#i if agent_name not in agent_colors else agent_colors[agent_name]
        agent_marker_index = 0#agent_color_index
    ...
    unit = "Cost" if use_cost else plot_y
    plot_label = "Cumulative" if cumulative else ""
    ...
    # Axis labels.
    x_axis_label = X_AXIS_LABEL if X_AXIS_LABEL is not None else x_axis_unit[0].upper() + x_axis_unit[1:] + ""
...
def plot_successes(results, experiment_dir, agents, plot_file_name="", conf_intervals=[], use_cost=False, cumulative=False, episodic=True, open_plot=True, track_disc_reward=False, plot_title = ''):
    '''
    Args:
        results (list of lists): each element is itself the reward from an episode for an algorithm.
        experiment_dir (str): path to results.
    ...
    # Make the plot.
    print_prefix = "\nAvg. cumulative reward" if cumulative else "% of goals reached"
    # For each agent.
    for i, agent_name in enumerate(agents):
        print('i:', i , agent_name)
        # Add figure for this algorithm.
    ...
    # Configure plot naming information.
    unit = "Cost" if use_cost else "% of goals reached"
    plot_label = "Cumulative" if cumulative else ""
    if "times" in experiment_dir:
        # If it's a time plot.
        unit = "Time"
...
def make_plots(data, experiment_agents, plot_file_name="", plot_title = '_', cumulative=True, use_cost=False, episodic=True, open_plot=True, track_disc_reward=False,plot_y = 'Reward'):
    '''
    Args:
        experiment_dir (str): path to results.
    ...
    # # Create plot.
    plot(avg_data, '', experiment_agents,
                ...
                episodic=episodic,
                open_plot=open_plot,
                track_disc_reward=track_disc_reward ,plot_title = plot_title, plot_y = plot_y)
    
    # # Average the data.
    # avg_data = average_data(success_data, cumulative=cumulative)
    # # Compute confidence intervals.
    # conf_intervals = compute_conf_intervals(success_data, cumulative=cumulative)
...
#     # # Grab agents.
#     # data_dir = args.dir
#     # agent_names = 'random' #_get_agent_names(data_dir)
...
#     # episodic =  True #_is_episodic(data_dir)
#     # track_disc_reward = True #_is_disc_reward(data_dir)
...
#     make_plots(data, ['random'], cumulative=False, episodic=True, track_disc_reward=False, open_plot=False)
#     pass
